module vertical_diffusion
  ...
  character(16) eddy_scheme                            ! Default set in phys_control.F90, use namelist to change
  ...
  character(16) shallow_scheme                         ! Shallow convection scheme
...
contains
  ...
  subroutine vd_readnl(nlfile)
    ...
    ! Local variables
    integer unitn, ierr
    character(*), parameter :: subname = 'vd_readnl'
  ...
  subroutine vertical_diffusion_init(pbuf2d)
    ...
    if (do_molec_diff) then
      call init_molec_diff(r8, pcnst, mwdry, avogad, errstring)
    ...
    if (vdiff_select(fieldlist_wet, 'u') /= '') call endrun(vdiff_select(fieldlist_wet, 'u'))
    if (vdiff_select(fieldlist_wet, 'v') /= '') call endrun(vdiff_select(fieldlist_wet, 'v'))
    if (vdiff_select(fieldlist_wet, 's') /= '') call endrun(vdiff_select(fieldlist_wet, 's'))
    ...
    constit_loop: do k = 1, pcnst
      ...
      if (vdiff_select(fieldlist_wet, 'q', k) /= '') call endrun(vdiff_select(fieldlist_wet, 'q', k))

      ! ----------------------------------------------- !
      ! Select constituents for molecular diffusion     !
      ! ----------------------------------------------- !
      if (cnst_get_molec_byind(k) == 'minor') then
        if (vdiff_select(fieldlist_molec,'q',k) /= '') call endrun(vdiff_select(fieldlist_molec, 'q', k))
      end if
    end do constit_loop
  ...
  subroutine vertical_diffusion_tend( &
    ...
    if (do_molec_diff) then
      if (waccmx_mode) then
        tint(:ncol,1) = 1.5_r8 * tint(:ncol,2) - 0.5_r8 * tint(:ncol,3)
      else
    ...
    if (.not. do_pbl_diags) then
      sl_prePBL(:ncol,:pver)  = s_tmp(:ncol,:) - latvap * q_tmp(:ncol,:,ixcldliq) - (latvap + latice) * q_tmp(:ncol,:,ixcldice)
      qt_prePBL(:ncol,:pver)  = q_tmp(:ncol,:,1) + q_tmp(:ncol,:,ixcldliq) + q_tmp(:ncol,:,ixcldice)
    ...
    if (do_molec_diff .and. .not. waccmx_mode) then
      ...
      dse_top(:ncol) = cpairv(:ncol,1,lchnk) * tint(:ncol,1) + gravit * state%zi(:ncol,1)
    else
      dse_top(:ncol) = 0
    end if
    ...
    if (.not. do_pbl_diags) then
      sl(:ncol,:pver)   = s_tmp(:ncol,:) - latvap * q_tmp(:ncol,:,ixcldliq) - (latvap + latice) * q_tmp(:ncol,:,ixcldice)
      ...
      slflx_cg(:ncol,pverp) = 0
      qtflx_cg(:ncol,pverp) = 0
      uflx_cg(:ncol,pverp)  = 0
      vflx_cg(:ncol,pverp)  = 0
    ...
    if ((eddy_scheme == 'diag_TKE' .or. eddy_scheme == 'SPCAM_m2005') .and. do_pseudocon_diff) then
      ...
      call positive_moisture(              &
        ...
        qv_pro(:ncol,pver:1:-1)          , &
        ql_pro(:ncol,pver:1:-1)          , &
        qi_pro(:ncol,pver:1:-1)          , &
        t_pro(:ncol,pver:1:-1)           , &
        s_pro(:ncol,pver:1:-1)           , &
        ptend%q(:ncol,pver:1:-1,1)       , &
        ptend%q(:ncol,pver:1:-1,ixcldliq), &
        ptend%q(:ncol,pver:1:-1,ixcldice), &
        ptend%s(:ncol,pver:1:-1))
    end if
    ...
    if (.not. do_pbl_diags) then
      ...
      call qsat(t_aftPBL(:ncol,:pver), state%pmid(:ncol,:pver), tem2(:ncol,:pver), ftem(:ncol,:pver))
      ...
      do m = 1, pcnst
        fixed_ubc: if (.not. cnst_fixed_ubc(m) .and. .not. cnst_fixed_ubflx(m)) then
           col_loop: do i = 1, ncol
              ...
              sum3 = 0
              do k = 1, pver
                if (cnst_get_type_byind(m) == 'wet') then
  ...
  subroutine positive_moisture(cp, xlv, xls, ncol, mkx, dt, qvmin, qlmin, qimin, &
    ...
    do i = 1, ncol
      ...
      if (dqv > 1.e-20_r8) then
        sum = 0._r8
        do k = 1, mkx
          if (qv(i,k) > 2 * qvmin) sum = sum + qv(i,k) * dp(i,k)
        end do
        aa = dqv * dp(i,1) / max(1.0e-20_r8, sum)
        if (aa .lt. 0.5_r8) then
          do k = 1, mkx
            if (qv(i,k) > 2 * qvmin) then
              dum        = aa * qv(i,k)
              qv(i,k)    = qv(i,k) - dum
