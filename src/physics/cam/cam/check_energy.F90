...
! Physics buffer indices
  ...
  type check_tracers_data
    real(r8), allocatable, dimension(:,:) :: tracer     ! initial vertically integrated total (kinetic + static) energy
    real(r8), allocatable, dimension(:,:) :: tracer_tnd ! cumulative boundary flux of total energy
    integer , allocatable, dimension(:  ) :: count      ! count of values with significant imbalances
  contains
    procedure :: init  => check_tracers_data_init
    procedure :: clear => check_tracers_data_clear
    final check_tracers_data_final
  end type check_tracers_data

contains

  subroutine check_tracers_data_init(this)

    class(check_tracers_data), intent(inout) :: this
  ...
  subroutine check_tracers_data_clear(this)

    class(check_tracers_data), intent(inout) :: this

    if (allocated(this%tracer    )) deallocate(this%tracer    )
    if (allocated(this%tracer_tnd)) deallocate(this%tracer_tnd)
    if (allocated(this%count     )) deallocate(this%count     )

  end subroutine check_tracers_data_clear

  subroutine check_tracers_data_final(this)

    type(check_tracers_data), intent(inout) :: this

    call this%clear()

  end subroutine check_tracers_data_final
  ...
  subroutine check_energy_readnl(nlfile)
    ...
    integer unitn, ierr
    character(*), parameter :: sub = 'check_energy_readnl'
...
!-----------------------------------------------------------------------
    ...
    real(r8) te(pcols,begchunk:endchunk,3) ! Total energy of input/output states (copy)
...
!-----------------------------------------------------------------------
    ...
    character(8) tracname                          ! tracername
    ...
    do m = 1,pcnst
      ...
      if (maxval(tracer_rer) > 1.0e-14_r8) then
  ...
  subroutine calc_te_and_aam_budgets(state, outfld_name_suffix)
    ...
    character(16) name_out1,name_out2,name_out3,name_out4,name_out5,name_out6
