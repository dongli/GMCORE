module zm_conv_intr
  ...
  public zm_conv_readnl   ! read namelist
  ...
  integer :: difzm_idx        = 0 ! Detrained convective cloud ice mixing ratio.
  integer :: dnlfzm_idx       = 0 ! Detrained convective cloud water num concen.
  integer :: dnifzm_idx       = 0 ! Detrained convective cloud ice num concen.
...
contains
  ...
  subroutine zm_conv_register()
    ...
    if (zmconv_microp) then
      ! Only add the number concentration fields if the microphysics is active.
    ...
    if (zmconv_org) then
      call cnst_add('ZM_ORG', 0.0_r8, 0.0_r8, 0.0_r8, ixorg, longname='organization parameter')
    end if
  ...
  subroutine zm_conv_readnl(nlfile)
    ...
    integer unitn, ierr
    character(*), parameter :: subname = 'zm_conv_readnl'
  ...
  subroutine zm_conv_tend(pblh    ,mcon     ,cme     , &
    ...
    !
    mcon(:ncol,:pver) = mcon(:ncol,:pver) * 100.0_r8 / gravit
    ...
    ! Evaporate some of the precipitation directly into the environment (Sundqvist)
    ...
    if (zmconv_org) then
      ptend_loc%q(:ncol,:pver,ixorg) = min(1.0_r8, max(0.0_r8, (50.0_r8 * 1000.0_r8 * 1000.0_r8 * abs(evapcdp(:ncol,:pver))) &
                                     - (state%q(:ncol,:pver,ixorg) / 10800.0_r8)))
  ...
  subroutine zm_conv_tend_2(state, ptend, ztodt, pbuf)
    ...
    real(r8), pointer, dimension(:,:,:) :: fracis  ! Fraction of transported species that are insoluble
    real(r8), pointer, dimension(:,:  ) :: mu      ! (pcols,pver)
    real(r8), pointer, dimension(:,:  ) :: eu      ! (pcols,pver)
    real(r8), pointer, dimension(:,:  ) :: du      ! (pcols,pver)
    real(r8), pointer, dimension(:,:  ) :: md      ! (pcols,pver)
    real(r8), pointer, dimension(:,:  ) :: ed      ! (pcols,pver)
    real(r8), pointer, dimension(:,:  ) :: dp      ! (pcols,pver)
    real(r8), pointer, dimension(:    ) :: dsubcld ! (pcols)
    integer , pointer, dimension(:    ) :: jt      ! (pcols)
    integer , pointer, dimension(:    ) :: maxg    ! (pcols)
    integer , pointer, dimension(:    ) :: ideep   ! (pcols)
  ...
  subroutine zm_aero_init(nmodes, nbulk, aero)
    ...
    character(20), allocatable :: aername(:)
    character(32) str32
    character(*), parameter :: routine = 'zm_conv_init'
    ...
    if (nmodes > 0) then
      ...
      do m = 1, nmodes
        ...
        do l = 1, aero%nspec(m)
          call rad_cnst_get_aer_props(0, m, l, density_aer=aero%specdens(l,m), hygro_aer=aero%spechygro(l,m))
        end do
      end do
    else if (nbulk > 0) then
      aero%scheme = 'bulk'
      ...
      do iaer = 1, aero%nbulk
        call rad_cnst_get_aer_props(0, iaer, aername=aername(iaer), num_to_mass_aer=aero%num_to_mass_aer(iaer))
  ...
  subroutine zm_conv_micro_outfld(conv, dnif, dnlf, lchnk, ncol)
    ...
    do k = 1, pver
      do i = 1, ncol
        if (conv%qice(i,k) > 0) then
          cice_snum(i,k) = 1
        else
          cice_snum(i,k) = 0
        end if
        if (conv%qliq(i,k) > 0) then
          cliq_snum(i,k) = 1
        else
          cliq_snum(i,k) = 0
        end if
        if (conv%qsnow(i,k) > 0) then
          csnow_snum(i,k) = 1
        else
          csnow_snum(i,k) = 0
        end if
        if (conv%qrain(i,k) > 0) then
          crain_snum(i,k) = 1
        else
          crain_snum(i,k) = 0
        end if

        if (conv%qnl(i,k) > 0) then
          qnl_snum(i,k) = 1
        else
          qnl_snum(i,k) = 0
        end if
        if (conv%qni(i,k) > 0) then
          qni_snum(i,k) = 1
        else
          qni_snum(i,k) = 0
        end if
        if (conv%wu(i,k) > 0) then
          wu_snum(i,k) = 1
        else
          wu_snum(i,k) = 0
        end if
